...
import type { Invoice as InvoiceType } from "../../types/invoice.types";  // Update this line
import SignupUnitManager from "@/components/modal/SignupUnitManager";
import Invoice from "@/components/modal/Invoice";
import UnitManagerGroupModal from "@/components/modal/UnitManagerGroupModal";

const UnitManagerHome = () => {
  // State Management
  const [activeSection, setActiveSection] = useState<'users' | 'invoices' | 'groups'>('users');
  const [isSignup, setIsSignup] = useState(false);
  const [showInvoiceModal, setShowInvoiceModal] = useState(false);
  const [showUnitManagerGroupModal, setShowUnitManagerGroupModal] = useState(false);
  ...
  const handleDeleteInvoice = async (invoiceId: string) => {
    try {
      ...
      if (response.success) {
        ...
        setFilteredInvoices(prev => prev.filter(invoice => invoice._id !== invoiceId));
  ...
  const handleViewGroupMember = (adminId: string) => {
  ...
  const getPaginatedInvoices = () => {
    const start = (invoiceCurrentPage - 1) * invoiceItemsPerPage;
    return filteredInvoices.slice(start, start + invoiceItemsPerPage);
  };
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="fixed w-64 h-screen bg-zinc-900 p-4">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-red-500">Unit Manager Panel</h2>
        </div>
      ...
      <div className="ml-64 flex-1 bg-black p-4 sm:p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white">
              {activeSection === 'users' ? 'Users' : 'Invoices'}
            </h1>
        ...
        {activeSection === 'groups' && (
          <>
            <div className="mb-4">
              <Button
                onClick={() => setShowUnitManagerGroupModal(true)}
                className="bg-red-700 hover:bg-red-800"
              >
                <Plus className="w-4 h-4 mr-2" />
                Add Unit Manager to Group
              </Button>
            </div>

            <div className="bg-zinc-900 rounded-lg p-4">
              {groupedAdmins.length === 0 ? (
                <p className="text-center text-gray-400 py-4">No unit managers in your group</p>
              ) : (
            ...
            {showUnitManagerGroupModal && (
              <UnitManagerGroupModal
                onClose={() => setShowUnitManagerGroupModal(false)}
                onUnitManagerAdded={() => {
                  fetchGroupedAdmins();
                  setShowUnitManagerGroupModal(false);
                }}
              />
            )}
          </>
